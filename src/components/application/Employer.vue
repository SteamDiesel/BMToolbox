	...
	<div class="">
		<div class="flex flex-wrap">
			<div class="w-1/5 flex justify-around items-center">
				<button
					...
					:class="{
						'bg-teal-400 font-semibold hover:bg-teal-300':
							employer.current,
					}"
				>
		...
		<div v-if="employer.current">
			...
			<div class="flex flex-wrap">
				<FormField
					class="w-1/3"
					@copy="copyClipboard('#' + employer.net_monthly_income)"
				>
					...
					<input
						...
						@keyup.87="
							employer.net_monthly_income =
								employer.net_monthly_income * 4.33
						"
						@keyup.70="
							employer.net_monthly_income =
								employer.net_monthly_income * 2.166
						"
						@keyup.81="
							employer.net_monthly_income =
								employer.net_monthly_income * 0.33
						"
						@keyup.89="
							employer.net_monthly_income =
								employer.net_monthly_income * 0.0833
						"
						@change="updateField"
						type="number"
				...
				<FormField
					v-if="payslip_avg_net_monthly"
					class="w-2/3"
					@copy="copyClipboard('#jjikk' + payslip_avg_net_monthly)"
				>
					<template v-slot:label>Pay Slip Confrimed Income</template>
					<div
						class="form-input pb-1 text-center font-semibold flex justify-around"
					>
						<span>${{ payslip_avg_net_weekly }}/w</span>
						<span> ${{ payslip_avg_net_monthly }}/m </span>
					</div>
				</FormField>
			</div>
...
<script>
	...
	export default {
		...
		computed: {
			payslip_time_weeks() {
				return moment(this.employer.payslip_date).diff(
					this.employer.payslip_start_date,
					"weeks"
				);
			},
			payslip_time_months() {
			...
			payslip_avg_net_monthly() {
				var result = Math.round(
					+this.employer.payslip_net_ytd / +this.payslip_time_months
				);
				if (result >= 10) {
					return result;
				} else {
			...
			payslip_avg_net_weekly() {
				var result = Math.round(
					+this.employer.payslip_net_ytd / +this.payslip_time_weeks
				);
				if (result >= 10) {
					return result;
				} else {
		...
		methods: {
			isCurrentSetStartDate() {
				var date = moment()
					.subtract(this.employer.years, "years")
					.subtract(this.employer.months, "months")
					.format("YYYY-MM-DD");
			...
			isCurrentSetTenure() {
				...
				var years = Math.floor(
					moment().diff(this.employer.start_date, "years")
				);
			...
			isPreviousSetEndDate() {
				if (!this.employer.end_date) {
					...
					this.employer.end_date = moment(
						this.employers[prev_index].start_date
					)
						.subtract(1, "days")
						.format("YYYY-MM-DD");
				}
			...
			isPreviousSetStartDate() {
				var date = moment(this.employer.end_date)
					.subtract(this.employer.years, "years")
					.subtract(this.employer.months, "months")
					.format("YYYY-MM-DD");
				this.employer.start_date = date;
