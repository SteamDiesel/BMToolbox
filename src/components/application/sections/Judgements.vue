<template>
	<Section>
		<Header title="Court Judgements">
			<button
				class="relative bg-gray-300 hover:bg-blue-200 p-2 shadow-lg rounded-full no-print"
				@click="pushToArray({person: person, type: 'court_judgement', array: person.court_judgements})"
			>
		...
		<Judgement
			class="mb-4"
			v-for="(court_judgement, index) in person.court_judgements"
			:key="'court_judgement'+index"
			:court_judgement="court_judgement"
		>
			<button
				class="relative bg-gray-300 hover:bg-red-200 p-2 shadow-lg rounded-full no-print text-xs"
				@click="dropFromArray({array: person.court_judgements, type: 'court_judgement', index: index, person: person, object: court_judgement})"
			>
			...
			<!-- Link to next person  -->
			<button
				v-show="!court_judgement.shared"
				:class="{ 'bg-blue-400': court_judgement.shared }"
...
import Judgement from "@/components/application/Judgement";
