<template>
	<Section>
		<Header title="Bankruptcies">
			<button
				class="relative bg-gray-300 hover:bg-blue-200 p-2 shadow-lg rounded-full no-print"
				@click="pushToArray({person: person, type: 'bankruptcy', array: person.bankruptcies})"
			>
		...
		<Bankruptcy
			class="mb-4"
			v-for="(bankruptcy, index) in person.bankruptcies"
			:key="'bankruptcy'+index"
			:bankruptcy="bankruptcy"
		>
			<button
				class="relative bg-gray-300 hover:bg-red-200 p-2 shadow-lg rounded-full no-print text-xs"
				@click="dropFromArray({array: person.bankruptcies, type: 'bankruptcy', index: index, person: person, object: bankruptcy})"
			>
			...
			<!-- Link to next person  -->
			<button
				v-show="!bankruptcy.shared"
				:class="{ 'bg-blue-400': bankruptcy.shared }"
...
import Bankruptcy from "@/components/application/Bankruptcy.vue";
